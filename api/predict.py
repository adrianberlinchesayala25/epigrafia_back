...
Detección de idioma mediante audio
Compatible con Vercel Python Runtime 2025

NOTA: Para inferencia con modelo ML, el frontend usa TensorFlow.js.
Este endpoint procesa el audio y devuelve características para análisis.
"""
...
# ============================================
# Vercel Serverless Handler (HTTP Class Style)
# ============================================
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        response = {
            ...
            "note": "ML inference is handled client-side with TensorFlow.js"
        }
    ...
    def do_POST(self):
        ...
        Returns: Audio metadata and confirmation for client-side ML processing
        """
        try:
            # Get content info
            ...
            result = self._process_audio(audio_data, filename)
    ...
    def _process_audio(self, audio_data: bytes, filename: str) -> dict:
        """
        Process uploaded audio
        Returns metadata for client-side inference
        """
        try:
            # Try to get audio info using soundfile if available
            audio_info = self._get_audio_info(audio_data, filename)
            
            # For demo/testing: return mock prediction
            # Real inference happens client-side with TensorFlow.js
            return {
                "success": True,
                "audio": {
                    "filename": filename,
                ...
                "message": "Audio received successfully. Use TensorFlow.js for inference.",
                "inference_mode": "client-side",
                # Fallback prediction for compatibility
                "language": {
                    "detected": "Procesando...",
                    "confidence": 0.0,
    ...
    def _get_audio_info(self, audio_data: bytes, filename: str) -> dict:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=self._get_extension(filename), delete=False) as tmp:
    ...
    def _parse_multipart(self, body: bytes, content_type: str) -> tuple:
        ...
        try:
            ...
            for part in parts:
                # Check if this is an audio/file part
                if b'audio' in part.lower() or b'file' in part.lower() or b'name="audio"' in part.lower():
                    ...
                    # Find the actual data (after double CRLF)
